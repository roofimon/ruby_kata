...
describe Captcha do
    context "string" do
        ...
        it "should be Nine + 1" do
            @captcha = Captcha.new(2, 9, 1, 1)
            expect(@captcha.string()).to eq 'Nine + 1'
        end
        it "should be One - 9" do
            @captcha = Captcha.new(2, 1, 2, 9)
            expect(@captcha.string()).to eq 'One - 9'
        end
        it "should be 9 + Two" do
            @captcha = Captcha.new(1, 9, 1, 2)
            expect(@captcha.string()).to eq '9 + Two'
        end
        it "should be 1 * Eight" do
            @captcha = Captcha.new(1, 1, 3, 8)
            expect(@captcha.string()).to eq '1 * Eight'
        end
    end
